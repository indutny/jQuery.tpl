		...
		$.template = function (str , args, name) {		
			...
			str = 
				str
					[replace]($tabs,"")
						[replace]($brackets,"\t")
							[replace]($literal,function(X , start , elem , end) {
								return start+((namespace["$" + varcount] = elem)?"$p($"+(varcount++)+");":"")+end;
							})
								[replace]($code,function(X,elem) {
									return (i = elem.match($modificator)) && ( i.f = modificators[ i[1] ])  ?
										i.f(elem.substr(i[0][length]))
										:
										elem;
								});
